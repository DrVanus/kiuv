...
struct TreemapView: View {
    ...
    var tileSpacing: CGFloat = 6
    /// Minimum area to show labels
    var labelThreshold: CGFloat = 5
    /// Animate layout updates
    var animationDuration: Double = 0.5
    /// Show legend below the map
    var showLegend: Bool = true
    /// Number of top coins to always label
    var topLabelCount: Int = 8
    /// Number of top coins to display individually
    var topCount: Int = 15

    /// Exponent for marketCap weighting in the layout (reduce dominance)
    var weightExponent: Double = 0.4

    /// Effective color bound: fixed ±10% to highlight moderate moves
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Canvas { context, size in
                // Fixed grouping: show top 10 coins individually, group the rest as “Others”
                let sortedAll = tiles.sorted { $0.marketCap > $1.marketCap }
                let displayCount = min(topCount, sortedAll.count)
                let topTiles = Array(sortedAll.prefix(displayCount))
                let smallTiles = sortedAll.dropFirst(displayCount)
                var displayTiles = topTiles
                ...
                // Use squarified treemap layout with exponent scaling for better aspect ratios
                let weights = displayTiles.map { pow($0.marketCap, weightExponent) }
                let rects = squarify(
                    items: displayTiles,
                    weights: weights,
                ...
                for (tile, rect) in zip(displayTiles, rects) {
                    ...
                    let roundedPath = Path(roundedRect: insetRect, cornerRadius: 6)
                    ...
                    if displayTiles.firstIndex(of: tile)! < topLabelCount || area > labelThreshold {
                        ...
                        context.draw(Text(attr), at: CGPoint(x: insetRect.midX, y: insetRect.midY))
                    }
                }
            }
