...
/// Response wrapper for the CoinStats news endpoint
fileprivate struct NewsResponse: Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        if let newsArray = try? container.decode([NewsArticle].self, forKey: .news) {
            news = newsArray
        } else if let dataArray = try? container.decode([NewsArticle].self, forKey: .data) {
            news = dataArray
        } else {
            news = []
        }
...
struct NewsArticle: Decodable, Identifiable {
    ...
    /// Use the article link as a stable identifier.
    var id: String { link }
}

/// Simple row for previewing NewsArticle
...
struct HomeView: View {
    ...
    @StateObject private var rssNewsVM = CryptoNewsFeedViewModel()

    @State private var showSettings = false
    @State private var showNotifications = false
    @State private var isEditingWatchlist = false  // For reordering
    // Removed unused showWatchlistOnly state property
    ...
    private var homeContentStack: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            eventsSection
            exploreSection
            // MARK: - Latest Crypto News (RSS)
            SectionHeading(text: "Latest Crypto News", iconName: "newspaper")
            CryptoNewsView()
                .environmentObject( rssNewsVM )
                .frame(height: 300)
                .listStyle(PlainListStyle())
                .padding(.horizontal, -16)
                .onAppear { rssNewsVM.loadNews() }
...
extension HomeView {
    ...
    private var heatMapSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            SectionHeading(text: "Market Heat Map", iconName: "square.grid.2x2")
            
            // DEBUG: show how many coins are driving the treemap
            Text("Market coins: \(marketVM.coins.count)")
                .font(.caption2)
                .foregroundColor(.white)
            
            // Treemap driven by live data
            TreemapView(
                tiles: marketVM.coins.map { coin in
                    HeatMapTile(
                        symbol: coin.symbol.uppercased(),
                        pctChange: coin.dailyChange,
...
// MARK: - NewsPreviewSection removed (no longer used)
