...
// MARK: - CryptoChartInterval Helpers
extension CryptoChartInterval {
    /// String interval value for TradingView widget
    var tvValue: String {
        switch self {
    ...
    /// Whether to hide the crosshair time label for long-term intervals
...
struct CoinDetailView: View {
    ...
    @State private var selectedInterval: CryptoChartInterval = .oneDay
...
// MARK: - Chart Section
    ...
    private var chartSection: some View {
        if selectedChartType == .cryptoSageAI {
            CoinDetailCustomChart(symbol: coin.symbol,
    ...
    private var intervalRow: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                ForEach(CryptoChartInterval.allCases, id: \.self) { interval in
                    Button(action: {
                        ...
                        Text(interval.rawValue)
                            ...
                            .background(
                                RoundedRectangle(cornerRadius: 12)
                                    .fill(selectedInterval == interval ? Color.yellow : Color.white.opacity(0.15))
                            )
...
struct CoinDetailCustomChart: View {
    let symbol: String
    let interval: CryptoChartInterval
