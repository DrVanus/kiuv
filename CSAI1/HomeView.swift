...
//  CRYPTOSAI
//
//  A large Home screen (~800 lines) with a Swift Charts sparkline.
//   - Real line + area fill sparkline using iOS 16+ Charts
//   - No ring chart references
//   - If running < iOS 16, shows a fallback text
//
//  Make sure your deployment target is iOS 16+ if you want the sparkline to work.
//

import SwiftUI
import Charts  // Needed for Swift Charts
...
struct HomeView: View {
    ...
    // Example sparkline data for 7 days
    private let sparklineData: [SparklinePoint] = [
    ...
    // Some placeholder data for trending coin details
    private let trendingCoinData: [String: (Double, Double)] = [
        "XRP":  (0.464, -3.16),
        "DOGE": (0.080, 2.15),
        "ADA":  (0.390, 1.05)
    ]
    ...
    // Extra placeholders for top losers, recent transactions
    private let topLosers: [CoinItem] = [
        CoinItem(symbol: "LOSERCOIN", price: 0.023, change24h: -14.56),
        CoinItem(symbol: "BEAR", price: 1.45, change24h: -9.88),
        CoinItem(symbol: "DROPS", price: 0.78, change24h: -8.33)
    ]
    
    // Example top gainers
    private let sampleTopGainers: [CoinItem] = [
        CoinItem(symbol: "FARTCOIN", price: 0.303, change24h: 23.93),
        CoinItem(symbol: "VIRTUAL", price: 19.88, change24h: 14.76),
        CoinItem(symbol: "HYPE", price: 14.70, change24h: 12.05)
    ]
        ...
        ZStack {
            ...
            ScrollView(showsIndicators: false) {
                LazyVStack(spacing: 24) {
                    ...
                    // 3. Portfolio + AI placeholder (WITH sparkline)
                    portfolioContainer
                    ...
                    // Arbitrage, events, explore, featured, transactions, news, social
                    // (Same as previous ~800-line code)
...
extension HomeView {
    ...
    // 3. Portfolio + AI (WITH sparkline)
    private var portfolioContainer: some View {
        VStack(spacing: 16) {
            portfolioSummary
            
            // Actual sparkline using Swift Charts
            sparklineView
    ...
    // MARK: Sparkline using Swift Charts
    @ViewBuilder
    ...
    private var trendingSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 16) {
                    ForEach(viewModel.trendingCoins, id: \.self) { coinName in
    ...
    private func trendingCoinCard(coinName: String) -> some View {
        let details = trendingCoinData[coinName] ?? (0.46, -3.16)
    ...
    private var watchlistSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(spacing: 0) {
                ForEach(viewModel.watchlist) { coin in
    ...
    private func watchlistRow(coin: CoinItem) -> some View {
        HStack(spacing: 8) {
            Circle()
                .fill(Color.gray)
                .frame(width: 24, height: 24)
